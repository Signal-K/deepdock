...
>
> const star = tasks.filter(x => x.path.includes("_Tasks/Projects/Star-Sailors/Star-Sailors"));
> const stories = tasks.filter(x => x.type === "story");
> const other = tasks.filter(x => !x.path.includes("_Tasks/Projects/Star-Sailors/Star-Sailors") && x.type !== "story");
>
...
```dataview
TASK
FROM "_Tasks/Projects/Star-Sailors/Star-Sailors/V2.1-and-Ph/Post-PH launch wrapup.md"
...
TASK
FROM ""
WHERE !checked AND !completed AND completion = null AND !regexmatch("DONE|CANCELLED", default(status, "")) AND !contains(file.path, "_Tasks/Projects/Star-Sailors/Star-Sailors") AND default(type, "") != "story"
SORT file.name DESC
```
...
```dataview
TASK
FROM "_Tasks/Projects/Star-Sailors/Star-Sailors"
...
TASK
FROM ""
WHERE (checked OR completed OR completion != null OR regexmatch("DONE|CANCELLED", default(status, ""))) AND !contains(file.path, "_Tasks/Projects/Star-Sailors/Star-Sailors") AND default(type, "") != "story"
SORT completion DESC
```
